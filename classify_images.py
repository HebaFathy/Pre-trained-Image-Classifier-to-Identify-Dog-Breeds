...
def classify_images(images_dir, results_dic, model):
    ...
    for x in results_dic:
        # classifier function takes inputs as path+filename, model and returns label
        label = classifier(images_dir+x, model)
        # stripping off whitespace from label
        label = label.lower().strip()
        
        # defines truth as pet image label 
        truth = results_dic[x][0]
        ...
        if truth in label:
            results_dic[x].extend((label, 1))
        else:
            results_dic[x].extend((label, 0))